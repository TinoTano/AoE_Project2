				...
				if (name->str == "TOWN CENTER") {
					...
					case BUILDINGMENU:
						if (building_state != BUILDINGMENU)
						{
							...
							if (create_unit_bt->current == CLICKUP)
							{
								...
								if (App->sceneManager->level1_scene->SpendResourcesIn(App->entityManager->unitsDB[ELF_VILLAGER]->cost)) 
								{
					...
					case BUILDINGCREATEUNITS:
						if (building_state != BUILDINGCREATEUNITS)
						{
							...
							{
								if(App->sceneManager->level1_scene->SpendResourcesIn(App->entityManager->unitsDB[ELVEN_ARCHER]->cost))
								{
							...
							{
								if (App->sceneManager->level1_scene->SpendResourcesIn(App->entityManager->unitsDB[ELVEN_LONGBLADE]->cost))
								{
							...
							{
								if (App->sceneManager->level1_scene->SpendResourcesIn(App->entityManager->unitsDB[GONDOR_HERO]->cost))
								{
