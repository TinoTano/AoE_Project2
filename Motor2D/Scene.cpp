...
bool Scene::PostUpdate()
{
	...
	if ((App->entityManager->player->Town_center->Life <= 0 && game_finished == false) || (App->entityManager->player->units.size() <= 0 && game_finished == false)) {
		game_finished = true;
		Label* defeat = (Label*)App->gui->CreateLabel("DEFEAT", -STARTING_CAMERA_X + 665, -STARTING_CAMERA_Y + 40, App->font->fonts[SIXTEEN]);
		...
		Label* victory = (Label*)App->gui->CreateLabel("VICTORY", -STARTING_CAMERA_X + 665, -STARTING_CAMERA_Y + 40, App->font->fonts[SIXTEEN]);
